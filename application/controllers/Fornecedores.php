...
class Fornecedores extends CI_Controller
{
	...
	public function edit($fornecedor_id = NULL)
	{
		...
		if (!$fornecedor_id || !$this->core_model->get_by_id('fornecedores', array('fornecedor_id' => $fornecedor_id))) {
			...
			$this->form_validation->set_rules('fornecedor_cnpj', '', 'trim|required|max_length[18]|callback_valida_cnpj');
			...
			$this->form_validation->set_rules('fornecedor_cep', '', 'trim|required|max_length[9]');
			...
			$this->form_validation->set_rules('fornecedor_complemento', '', 'trim|required|max_length[45]');
			...
			if ($this->form_validation->run()) {
				...
				if ($this->db->table_exists('produtos')) {
					...
					if ($fornecedor_ativo == 0 && $this->core_model->get_by_id('produtos', array('produto_fornecedor_id' => $fornecedor_id, 'fornecedor_ativo' => 1))) {
						$this->session->set_flashdata('info', 'Esta categoria não pode ser desativada, pois esta sendo utilizada em Produtos');
						redirect('categorias');
					}
				}
	...
	public function add()
	{
		...
		$this->form_validation->set_rules('fornecedor_cnpj', '', 'trim|required|max_length[18]|is_unique[fornecedores.fornecedor_cnpj]');
	...
	public function check_fornecedor_telefone($fornecedor_telefone)
	{
		...
		if ($this->core_model->get_by_id('fornecedores', array('fornecedor_telefone' => $fornecedor_telefone, 'fornecedor_id !=' => $fornecedor_id))) {
			$this->form_validation->get_message('check_fornecedor_telefone', 'Esse telefone já existe');
			return false;
		} else {
			return true;
		}
	...
	public function check_fornecedor_celular($fornecedor_celular)
	{
		...
		if ($this->core_model->get_by_id('fornecedores', array('fornecedor_celular' => $fornecedor_celular, 'fornecedor_id !=' => $fornecedor_id))) {
			$this->form_validation->get_message('check_fornecedor_celular', 'Esse celuar já existe');
			return false;
		} else {
			return true;
		}
