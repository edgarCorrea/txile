...
class Clientes extends CI_Controller
{
	...
	public function index()
	{
		$data = array(
			...
			'clientes' => $this->core_model->get_all('clientes'), // get all users
		);
	...
	public function add()
	{
		...
		if ($cliente_tipo == 1){
			$this->form_validation->set_rules('cliente_cpf', '', 'trim|required|max_length[14]|is_unique[clientes.cliente_cpf_cnpj]|callback_valida_cpf');
		}else{
			$this->form_validation->set_rules('cliente_cnpj', '', 'trim|required|max_length[18]| is_unique[clientes.cliente_cpf_cnpj]|callback_valida_cnpj');
		}
		...
		if (!empty($this->input->post('cliente_celular'))){
			$this->form_validation->set_rules('cliente_celular', '', 'trim|required|max_length[15]|is_unique[clientes.cliente_celular]|callback_check_celular');
		}

		$this->form_validation->set_rules('cliente_celular', '', 'trim|required|max_length[15]');
		$this->form_validation->set_rules('cliente_cep', '', 'trim|required|max_length[9]');
		...
		$this->form_validation->set_rules('cliente_complemento', '', 'trim|required|max_length[45]');
		$this->form_validation->set_rules('cliente_cidade', '', 'trim|required|max_length[45]');
		...
		if ($this->form_validation->run()) {
			...
			/*
							...
							[cliente_data_nascimento] => 1983 - 01 - 13
							   [cliente_rg_ie] => 555555555123123
							[cliente_cnpj] => 58.962.064 / 0001 - 53
							[cliente_email] => edgar_0483@hotmail . com
							[cliente_telefone] => (47) 9976 - 3482
							[cliente_celular] => (47) 9976 - 3482
							[cliente_endereco] => asaS
							[cliente_numero_endereco] => 12
							[cliente_complemento] => aSas
							[cliente_bairro] => as
							   [cliente_cep] => 89254 - 152
							[cliente_cidade] => as
							[cliente_estado] => as
							[cliente_ativo] => 1
							[cliente_obs] =>  adsadasd
							[cliente_tipo] => 2
							[cliente_id] => 1
	...
	public function edit($cliente_id = null)
	{
		...
		if (!$cliente_id || !$this->core_model->get_by_id('clientes', array('cliente_id' => $cliente_id))) {
			...
			$cliente_tipo = $this->input->post('cliente_tipo');
			if ($cliente_tipo == 1){
				$this->form_validation->set_rules('cliente_cpf', '', 'trim|required|max_length[14]|callback_valida_cpf');
			}else{
				$this->form_validation->set_rules('cliente_cnpj', '', 'trim|required|max_length[18]|callback_valida_cnpj');
			}
...
*/
	...
	public function check_rg_ie($cliente_rg_ie){
		...
		if ($this->core_model->get_by_id('clientes', array('cliente_rg_ie' => $cliente_rg_ie,'cliente_id !='=>$cliente_id))){
			$this->form_validation->set_message('check_ig_ie','Esse documento já existe');
			return false;
		}else{
			return true;
		}
	...
	public function check_telefone($cliente_telefone){
		...
		if ($this->core_model->get_by_id('clientes', array('cliente_telefone' => $cliente_telefone, 'cliente_id !='=> $cliente_id))){
			$this->form_validation->get_message('check_telefone', 'Esse telefone já existe');
			return false;
		}else{
			return true;
		}
	...
	public function check_celular($cliente_celular){
		...
		if ($this->core_model->get_by_id('clientes', array('cliente_celular' => $cliente_celular, 'cliente_id !='=> $cliente_id))){
			$this->form_validation->get_message('cliente_celular', 'Esse telefone já existe');
			return false;
		}else{
			return true;
		}
